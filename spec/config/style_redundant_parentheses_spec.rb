...
RSpec.describe "Style/RedundantParentheses configuration" do
  it "disables autocorrection only for the Ruby 1.8-compatible config" do
  ...
  it "does not override Standard's style for modern configs" do
    config = RuboCop::ConfigLoader.load_file(File.expand_path("../../config/ruby-3.4.yml", __dir__))

    expect(config["Style/RedundantParentheses"]).not_to include("AutoCorrect" => false)
  end
end
